...
class BaseExtractor:
    ...
    def _create_extraction_strategy(self) -> LLMExtractionStrategy:
        ...
        """
        # Add example data to help the LLM understand the expected format
        example_data = {
            "products": [
                {
        ...
        # Enhanced instructions to ensure proper data extraction
        enhanced_instructions = f"""
{self.site_config.extraction_instructions}

IMPORTANT: Make sure to extract the following required fields for each product:
1. name - The full product name (required)
2. price.current_price - The current price as a string including currency symbol (required)
3. source_site - Will be automatically added

Format the price correctly as an object with at least the current_price field.
                ...
                for product_data in products_data:
                    ...
                    try:
                        ...
                        if 'price' not in product_data or not product_data['price']:
                            product_data['price'] = {'current_price': 'N/A'}
                        elif isinstance(product_data['price'], str):
                            product_data['price'] = {'current_price': product_data['price']}
                        elif isinstance(product_data['price'], dict):
                            if 'current_price' not in product_data['price'] or not product_data['price']['current_price']:
                                product_data['price']['current_price'] = 'N/A'
                        else:
                            product_data['price'] = {'current_price': str(product_data.get('price', 'N/A'))}

                        # Ensure current_price is a string
                        if not isinstance(product_data['price']['current_price'], str):
                            product_data['price']['current_price'] = str(product_data['price']['current_price'])
                        ...
                        # Add partial product data anyway with guaranteed valid fields
                        try:
                            products.append(Product(
                                name=product_data.get('name', 'Unknown Product'),
                                price={
                                    'current_price': str(product_data.get('price', {}).get('current_price', 'N/A'))
                                    if isinstance(product_data.get('price'), dict) else str(product_data.get('price', 'N/A'))
                                },
                                source_site=self.site_config.name
                            ))
                        except Exception as inner_e:
                            print(f"Failed to create fallback product: {inner_e}")
